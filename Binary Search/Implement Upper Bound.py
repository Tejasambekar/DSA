def getFloorAndCeil(arr, n, x):
    ...
    while i <= j:  # Loop until i is less than or equal to j
        ...
        if arr[mid] == x:  # If the middle element is equal to x
            return [arr[mid], arr[mid]]  # Return a list with the middle element as both floor and ceil
        if arr[mid] < x:  # If the middle element is less than x
            floor = arr[mid]  # Update the floor variable to the middle element
            i = mid + 1  # Move the left pointer to mid + 1
        else:  # If the middle element is greater than x
            ceil = arr[mid]  # Update the ceil variable to the middle element
